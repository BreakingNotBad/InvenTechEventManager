...
namespace Service
{
    public class PackageService : IPackageService
    {
        ...
        public async Task<PackageDto> CreatePackageAsync(CreatePackageDto packageDto)
        {
            ...
            {
                NewPackage.EquipmentSets.Add(
                    new EquipmentSet { EquipmentId = item.EquipmentId, Quantity = item.Quantity }
                );
            }
        ...
        public async Task<PackageDto> UpdatePackageAsync(int id, UpdatePackageDto packageDto)
        {
            ...
            {
                var existingSet = existingSets.FirstOrDefault(x =>
                    x.EquipmentId == set.EquipmentId
                ); // หาในรายการเก่าว่ามีชิ้นนี้อยู่ไหม
                ...
                {
                    var equipmentExists =
                        await _repo.Equipment // เช็คก่อนว่า อุปกรณ์ที่เพิ่มมานั้น มีอยู่จริงไหม
                    ...
                    existingPackage.EquipmentSets.Add(
                        new EquipmentSet // เพิ่มใหม่
                        {
