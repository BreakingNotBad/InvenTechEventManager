    ...
    public class UpdateCompanyValidator : AbstractValidator<UpdateCompanyDto>
    {
        public UpdateCompanyValidator()
        {
            ...
            RuleFor(x => x.Longitude)
                .InclusiveBetween(-180m, 180)
                .WithMessage("Longitude must be between -180 and 180")
                .When(x => x.Longitude.HasValue);

            RuleFor(x => x.CompanyContacts)
                .Must(CompanyContact => CompanyContact.Count(c => c.IsPrimary) <= 1)
                .WithMessage("Only one primary contact is allowed.");

            RuleForEach(x => x.CompanyContacts).SetValidator(new UpdateCompanyContactValidator());
        }
    }
}
